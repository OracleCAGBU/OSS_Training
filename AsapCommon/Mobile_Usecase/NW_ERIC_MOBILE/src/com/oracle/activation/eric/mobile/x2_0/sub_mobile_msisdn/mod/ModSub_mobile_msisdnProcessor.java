...
public class ModSub_mobile_msisdnProcessor implements ModSub_mobile_msisdnProcessorInterface {
	...
	public void execute(ModSub_mobile_msisdnInput parms, ModSub_mobile_msisdnOutput output,
		...
		try {
			...
			logger.logDebug("ModSub_mobile_voiceProcessor: execute() - Completed step A of execution.");
			...
			String old_msisdn = parms.getOLD_MSISDN();
			String msisdn = parms.getMSISDN();

			HashMap<String, String> hm = new HashMap<>();

			hm.put(MobConstants.SERVICE, "update_msisdn");
			hm.put(MobConstants.MSISDN, msisdn);
			hm.put(MobConstants.OLD_MSISDN, old_msisdn);
			...
			if (!systemParameters.isLoopback()) {
				responseCode = connHdlr.sendRequest(hm,MobConstants.MOVE,true);
			}
			else
			{
				responseCode = connHdlr.sendRequest(hm,MobConstants.MOVE, false);
			}
		}
