...
declare namespace invtec                            = "http://xmlns.oracle.com/communications/inventory/webservice/technical";
...
let $eResponseMessage                   := $soapBody/invtec:calculateTechnicalActionsResponse
let $eBusinessInteractionUpdate         := systeminteractionmodule:createOrderDataUpdateForCTA($eResponseMessage, $eOrderData)
let $sBusinessInteractionUpdate         := saxon:serialize($eBusinessInteractionUpdate,<xsl:output method="xml" omit-xml-declaration="yes" indent="yes" saxon:indent-spaces="4"/>)

return
    (
        log:info($log, fn:concat('CalculateTechnicalAction Response : ', $sBusinessInteractionUpdate)),
        automator:setUpdateOrder($automator, "true"),
        context:updateOrderData($context,$sBusinessInteractionUpdate),
