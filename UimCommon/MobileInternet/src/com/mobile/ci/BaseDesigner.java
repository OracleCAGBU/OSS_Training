...
import com.mobile.utils.DeviceHelper;
		...
		try {
			...
			if (serviceAction.equalsIgnoreCase(Constants.SA_CREATE)) {
				...
				scvConVers = designer.designAdd(scvConVers, orderItem);
					...
					serviceAction.equalsIgnoreCase(Constants.SA_CHANGEPAYTYPE) || 
					serviceAction.equalsIgnoreCase(Constants.SA_CHANGEMSISDN)) {
	...
	private void makeProcessChildServiceOrder(ServiceConfigurationVersion scvConVers, BusinessInteractionItemType orderItem) throws ValidationException {
		...
		if(serviceSpec.endsWith(Constants.CFS)){
			...
			if(serviceAction.equals(Constants.SA_CREATE)){
				...
				serviceOrder = designManager.makeChildServiceOrder(serviceAction, orderItem.getParameterArray(), rfsSpecName, null, Constants.SPEC_MOBILE, extObjId);
				...
				serviceOrder = designManager.makeChildServiceOrder(serviceAction, orderItem.getParameterArray(), rfsSpecName, rfs, Constants.SPEC_MOBILE, extObjId); //serviceNumber
...
//				}
	...
	public void beforeComplete(ServiceConfigurationVersion scvConVers) throws ValidationException {
		...
		try {
			...
			String rfsServiceAction = UimHelper.getConfigItemCharValue(latestSCV, Constants.PARAM_PROPERTIES_CI, Constants.PARAM_SERVICE_ACTION); 
		...
		try {
			...
			for(ServiceConfigurationItem  configItem : configItemList) {
				if(configItem.getConfigType().equals(oracle.communications.inventory.api.entity.ConfigurationType.ITEM)
					...
					if(!configItem.getName().equals(Constants.PARAM_PROPERTIES_CI)) {
	...
	protected boolean isSuspending(ServiceConfigurationVersion scvConVers) throws ValidationException {		
		...
		if(scvConVers.getService().getSpecification().getName().endsWith("CFS")){	
			...
			if(null!=rfs) {
				...
				serviceAction = UimHelper.getConfigItemCharValue(latestSCV, Constants.PARAM_PROPERTIES_CI, Constants.PARAM_SERVICE_ACTION); 
			}
			...
			serviceAction = UimHelper.getConfigItemCharValue(scvConVers, Constants.PARAM_PROPERTIES_CI, Constants.PARAM_SERVICE_ACTION);
		}
	...
	protected void setPropertiesCI(ServiceConfigurationVersion scvConVers, ConfigurationItemType configItemType) throws ValidationException {
		...
		for(ConfigurationItemPropertyType prop: propList)
		{
			...
			String existingValue = UimHelper.getConfigItemCharValue(scvConVers, Constants.PARAM_PROPERTIES_CI, prop.getName());
	...
	public static BaseDesigner initializeDesigner(String specName, BaseDesigner designer){
		...
		if(specName.equals(Constants.SPEC_MOBILEINTERNET_CFS)){
			designer = new MobileInternetCfsDesigner();
		} else if (specName.equals(Constants.SPEC_MOBILEINTERNET_RFS)) {
			designer = new MobileInternetRfsDesigner();
		}
	...
	public String findnManageServicePort(ServiceConfigurationVersion scvConVers, ConfigurationItemType configItemType) throws ValidationException{
		...
		if(!Utils.isEmpty(virtualList)) {
			...
			UimHelper.setConfigItemCharValue(scvConVers, Constants.PARAM_PROPERTIES_CI, Constants.PARAM_ACTIVE_DEVICE_NAME, activeDeviceName);
			UimHelper.setConfigItemCharValue(scvConVers, Constants.PARAM_PROPERTIES_CI, Constants.PARAM_DEVICE_IPADDRESS, deviceIPAddress);
		}else {
	...
	public String getPEName(BusinessInteractionItemType orderItem){
		...
		for(ConfigurationItemType configItemType: configItemTypeList){
			if(configItemType.getName().equals(Constants.PARAM_PROPERTIES_CI)){					
				List<ConfigurationItemPropertyType> propList = configItemType.getPropertyList();
				for(ConfigurationItemPropertyType prop: propList){
	...
	public void manageOldnNewPropertiesCI(ServiceConfigurationVersion scvConVers) throws ValidationException {
		...
		UimHelper.setConfigItemCharValue(scvConVers, Constants.PARAM_PROPERTIES_CI, Constants.PARAM_DATAIP, deviceIPAddress);
		DesignManager designManager = DesignHelper.makeDesignManager();
		ServiceConfigurationItem pSci = designManager.aquireConfigItem(scvConVers, Constants.PARAM_PROPERTIES_CI);
